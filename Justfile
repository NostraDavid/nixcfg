...
format-shfmt:
  @git ls-files -z -- '*.sh' ':(glob)**/.bashrc' ':(glob)**/.bash_aliases' | grep -zEv '^dotfiles/agents/\.agents/skill-originals/polars-inc/' | xargs -0 --no-run-if-empty shfmt -i 4 -w

format-stylua:
  @git ls-files -z -- '*.lua' | xargs -0 --no-run-if-empty stylua

format-ruff:
  @git ls-files -z -- '*.py' 'dotfiles/git/.config/git/hooks/commit-msg' | grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' | xargs -0 --no-run-if-empty ruff format
...
check:
  ...
  @git ls-files -z -- '*.sh' ':(glob)**/.bashrc' ':(glob)**/.bash_aliases' | grep -zEv '^dotfiles/agents/\.agents/skill-originals/polars-inc/' | xargs -0 --no-run-if-empty shfmt -i 4 -d
  @git ls-files -z -- '*.lua' | xargs -0 --no-run-if-empty stylua --check
  @git ls-files -z -- '*.py' 'dotfiles/git/.config/git/hooks/commit-msg' | grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' | xargs -0 --no-run-if-empty ruff format --check
...
check-shfmt-files *files:
  @printf '%s\0' {{files}} | { grep -zEv '^dotfiles/agents/\.agents/skill-originals/polars-inc/' || true; } | xargs -0 --no-run-if-empty shfmt -i 4 -d

check-stylua-files *files:
  @stylua --check {{files}}

check-ruff-format-files *files:
  @printf '%s\0' {{files}} | { grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' || true; } | xargs -0 --no-run-if-empty ruff format --check
...
lint-ruff:
  @git ls-files -z -- '*.py' 'dotfiles/git/.config/git/hooks/commit-msg' | grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' | xargs -0 --no-run-if-empty ruff check

lint-ruff-files *files:
  @printf '%s\0' {{files}} | { grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' || true; } | xargs -0 --no-run-if-empty ruff check

lint-shellcheck:
  @git ls-files -z -- '*.sh' '.bashrc' '.bash_aliases' | grep -zEv '^dotfiles/agents/\.agents/skill-originals/polars-inc/' | xargs -0 --no-run-if-empty shellcheck --severity=error

lint-shellcheck-files *files:
  @printf '%s\0' {{files}} | { grep -zEv '^dotfiles/agents/\.agents/skill-originals/polars-inc/' || true; } | xargs -0 --no-run-if-empty shellcheck --severity=error

lint-markdown:
  @git ls-files -z -- '*.md' | grep -zEv '^(docs/agentskills\.io/|dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/)' | xargs -0 --no-run-if-empty markdownlint --disable MD013 MD040 MD041 --

lint-markdown-files *files:
  @printf '%s\0' {{files}} | { grep -zEv '^dotfiles/agents/\.agents/(skill-sources|skill-originals/polars-inc)/' || true; } | xargs -0 --no-run-if-empty markdownlint --disable MD013 MD040 MD041 --
