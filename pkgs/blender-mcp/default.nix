...
python3Packages.buildPythonApplication {
  ...
  postUnpack = ''
    sourceRoot="$sourceRoot/mcp"
  '';
  build-system = [python3Packages.setuptools];
  dependencies = with python3Packages; [docutils mcp pyyaml];
  postInstall = ''
    mkdir -p "$out/share/blender-mcp"
    cp -r ../addon/blender_mcp_addon "$out/share/blender-mcp/mcp"
  '';
  pythonImportsCheck = ["blmcp"];
  meta = {
    description = "Official Blender Lab MCP server and Blender extension";
