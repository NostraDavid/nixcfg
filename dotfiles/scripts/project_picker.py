...
def prompt_for_worktree_name(base_branch: str) -> str | None:
    proc = run(
        [
            "fzf",
            "--disabled",
            ...
            "--header",
            f"Create from branch {base_branch}",
        ],
        input_text="Type a name, then press Enter\n",
    )
    if proc.returncode != 0:
        return None
...
def list_branch_refs(bare_dir: Path) -> list[str]:
    proc = run(
        [
            "git",
            "--git-dir",
...
def list_tag_refs(bare_dir: Path) -> list[str]:
    proc = run(
        [
            "git",
            "--git-dir",
    ...
    return sorted(tag for tag in proc.stdout.splitlines() if tag)
...
def worktree_label(path: Path, kind: str, ref: str) -> str:
    if kind == "branch":
        return f"{path.name}  [branch: {ref}]"
    if kind == "tag":
        return f"{path.name}  [tag: {ref}]"
    return f"{path.name}  [detached: {ref[:12]}]"


def list_registered_worktrees(bare_dir: Path) -> list[WorktreeChoice]:
    proc = run(
        [
            "git",
            "--git-dir",
    ...
    def append_entry() -> None:
        ...
        if path.name in RESERVED_WORKTREE_NAMES:
            return
        if "branch" in entry:
            branch = entry["branch"].removeprefix("refs/heads/")
            choices.append(
                WorktreeChoice(
                    worktree_label(path, "branch", branch),
                    path,
                    "branch",
                    branch,
                    False,
        ...
        choices.append(
            WorktreeChoice(
                worktree_label(path, "detached", head),
                path,
                "detached",
                head,
                False,
...
def list_worktrees(
    repo_root: Path, *, refresh_empty: bool = False
) -> list[WorktreeChoice]:
    choices: list[WorktreeChoice] = []
    ...
    if branches_root.exists():
        for path in sorted(branches_root.rglob("*")):
            if path.is_dir() and (path / ".git").exists():
                raw_label = str(path.relative_to(branches_root))
                branch = "master" if raw_label == "master" else raw_label
                label = worktree_label(path, "branch", branch)
                append_choice(
                    choices,
                    seen_labels,
                    WorktreeChoice(label, path, "branch", branch, False),
                )
...
def find_active_worktree(
    worktrees: list[WorktreeChoice],
    cwd: Path,
) -> WorktreeChoice | None:
    existing_branches = [
        choice
        for choice in worktrees
        if choice.kind == "branch" and (choice.path / ".git").exists()
    ]
    cwd = cwd.resolve(strict=False)

    current = [
        choice
        for choice in existing_branches
        if cwd == choice.path or cwd.is_relative_to(choice.path)
    ]
    if current:
        return max(current, key=lambda choice: len(choice.path.parts))

    for choice in existing_branches:
        if choice.path.name == "trunk":
            return choice

    return None


def order_worktree_labels(
    worktrees: list[WorktreeChoice],
    new_worktree_label: str | None,
) -> list[str]:
    """Order labels for fzf's default bottom-up layout."""
    trunk_labels = [
        choice.label
        for choice in worktrees
        if choice.kind == "branch" and choice.path.name == "trunk"
    ]
    trunk_label_set = set(trunk_labels)
    other_labels = sorted(
        (choice.label for choice in worktrees if choice.label not in trunk_label_set),
        key=str.casefold,
        reverse=True,
    )

    labels = trunk_labels
    if new_worktree_label is not None:
        labels.append(new_worktree_label)
...
def resolve_branch_name(bare_dir: Path, branch_label: str) -> str | None:
    ...
    for branch in candidates:
        if local_branch_exists(bare_dir, branch) or remote_branch_exists(
            bare_dir, branch
        ):
            return branch
    return None
...
def create_worktree_from_branch(
    ...
    *,
    ...
    bare_dir = repo_root / BARE_REPO_DIR
    if not bare_dir.exists():
        print("Creating worktrees requires a worktree.git repository.", file=sys.stderr)
        return None
    ...
    if check_proc.returncode != 0:
        print(
            check_proc.stderr or f"Invalid branch name: {branch}\n",
            end="",
            file=sys.stderr,
        )
        return None
    ...
    base_head_proc = run(["git", "-C", str(base.path), "rev-parse", "--verify", "HEAD"])
    if base_head_proc.returncode != 0:
        print(base_head_proc.stderr or base_head_proc.stdout, end="", file=sys.stderr)
        return None

    base_head = base_head_proc.stdout.strip()
    if dry_run:
        return target
    ...
    def repo_matches(repo_root: Path) -> list[Path]:
        ...
        for choice in list_worktrees(repo_root):
            ...
            aliases = {
                ...
                choice.path.name,
                choice.ref.strip("/"),
                sanitize_worktree_name(choice.ref),
            }
            if worktree_query in aliases and choice.path.exists():
                matches.append(choice.path)
        return matches
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Select a repo worktree and print its path."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Select and validate a target without creating or updating worktrees.",
    )
...
def main() -> int:
    ...
    worktrees = list_worktrees(repo_root, refresh_empty=not args.dry_run)
    if not worktrees:
        print(f"No worktrees found for {repo_label}.", file=sys.stderr)
        return 1

    worktree_by_label = {choice.label: choice for choice in worktrees}
    active_worktree = find_active_worktree(worktrees, Path.cwd())
    new_worktree_label = None
    if active_worktree is not None and (repo_root / BARE_REPO_DIR).exists():
        new_worktree_label = f"{NEW_WORKTREE_PREFIX} {active_worktree.ref}"
    worktree_labels = order_worktree_labels(worktrees, new_worktree_label)

    worktree_label = choose_with_fzf(
        worktree_labels,
        prompt="Select worktree: ",
    )
    if worktree_label is None:
        print("No worktree selected.", file=sys.stderr)
        return 1

    if new_worktree_label is not None and worktree_label == new_worktree_label:
        assert active_worktree is not None
        branch = prompt_for_worktree_name(active_worktree.ref)
        if branch is None:
            print("No branch/worktree name entered.", file=sys.stderr)
            return 1

        target = create_worktree_from_branch(
            repo_root,
            active_worktree,
            branch,
    ...
    choice = worktree_by_label[worktree_label]
    if not ensure_worktree(repo_root, choice, dry_run=args.dry_run):
        print(f"Worktree does not exist: {choice.path}", file=sys.stderr)
        return 1
