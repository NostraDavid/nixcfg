...
De skillbronnen staan per herkomst onder `.agents/skill-sources`. Die bronmap is
bewust niet zelf een discoverylocatie. Home Manager projecteert elke skill
...
| [`local`](.agents/skill-sources/local/SOURCE.md)                     | 19     | Per skill of workflowfamilie vastgelegd in het bronbestand                                          |
...
gearchiveerd onder [`skill-originals`](.agents/skill-originals/README.md). Die
snapshots worden niet naar clients gekoppeld en kunnen met het meegeleverde
`compare.sh` naast de actieve varianten worden gelegd.
...
bewust draagbaar gemaakt en moeten bij een upstream-update opnieuw tegen de
...
- Codex gebruikt de ingebouwde skills uit `~/.codex/skills/.system`; de 57
  overige skills worden individueel gekoppeld onder `~/.codex/skills`.
- GitHub Copilot krijgt alle 62 skills als individuele koppelingen onder
  `~/.copilot/skills`.
