...
description: "Use when the user asks to configure a repository for Matt Pocock's engineering skills, or an invoked workflow requires missing issue-tracker, triage-label, or domain-document configuration. Do not trigger merely because these skills are installed."
---
...
> `to-tickets`, `triage`, `to-spec`, and `wayfinder` read from and write to it —
> they need to know whether to call `gh issue create`, write a markdown file
> under `.scratch/`, or follow some other workflow you describe. Pick the place
> you actually track work for this repo.
...
`triage` skill isn't installed (exploration told you) — an uninstalled skill
needs no labels.
...
tracker already uses other names (e.g. `bug:triage` for `needs-triage`) —
collect the overrides so `triage` applies existing labels instead of creating
duplicates.
...
Never create a second agent-instruction file when either one already exists —
