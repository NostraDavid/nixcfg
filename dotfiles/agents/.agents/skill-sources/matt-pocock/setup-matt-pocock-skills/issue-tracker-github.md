...
- **List issues**:
  ...
  [.labels[].name], comments: [.comments[].body]}]'`
...
- **List external PRs for triage**:
  ...
  number,title,body,labels,author,authorAssociation,comments`
...
- **Frontier query**: list the map's open children
  (`gh issue list --state
  open`, scoped to the map's sub-issues / task list),
...
- **Resolve**: `gh issue comment <n> --body "<answer>"`, then
  `gh issue close
  <n>`, then append a context pointer (gist + link) to the
