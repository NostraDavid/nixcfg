...
10. **HITL Python CLI.** Last resort. If a human must click, drive _them_ with
    `scripts/hitl_loop.py` so the loop is still structured. Read
    [HITL-LOOP.md](HITL-LOOP.md), write a TOML plan, preview it with
    `--dry-run`, then run it. Captured output feeds back to you.
