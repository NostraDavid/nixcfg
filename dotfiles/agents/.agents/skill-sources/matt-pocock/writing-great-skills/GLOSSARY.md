...
A skill whose client policy allows implicit invocation, so the agent can fire it
autonomously — and the human can still type its name, so model-invocation always
_includes_ user reach. It pays **context load** in exchange for discoverability
and can be reached by other skills. A model-invoked skill whose content is all
**reference** is also one home for shared reference. Pick model-invocation only
when the agent or another skill must reach it on its own.

_Avoid_: ability, tool, capability

### User-Invoked

A skill whose client policy disables implicit invocation, making it reachable
only by the human typing its name (user-_only_, where **model-invoked** is
user-_and-agent_). It trades agent-discoverability for **cognitive load**. The
required description remains a short human-facing summary, while
`policy.allow_implicit_invocation: false` in `agents/openai.yaml` carries the
Codex policy.
...
The skill's required machine-readable summary and, for a **model-invoked**
skill, its trigger. Client policy decides the invocation axis. A model-facing
description names the branches that should trigger the skill; a user-facing
description stays concise because the human already chose it.
...
its trigger metadata, spending both tokens and attention. User-invoked skills
reduce this cost by disabling implicit invocation. Context load is the brake on
