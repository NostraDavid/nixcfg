---
name: to-tickets
description: "Use when the user asks to split a plan, spec, or conversation into implementable tickets with dependencies for the configured issue tracker. Do not trigger for implementing existing tickets or exploring unresolved architectural decisions."
---
...
- **A real issue tracker (GitHub, Linear, …)** → publish one issue per ticket in
  dependency order (blockers first) so each ticket's blocking edges can
  reference real identifiers. Use the platform's native blocking / sub-issue
  relationship where it has one; otherwise set each ticket's "Blocked by" to the
  blocking issues. Apply the `ready-for-agent` triage label unless instructed
...
Work the frontier one ticket at a time with `$implement`, clearing context
between tickets.
