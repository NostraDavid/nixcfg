---
name: skill-creator
description: "Use when the user asks to create a new Codex skill or edit an existing skill's triggers, instructions, tool integrations, or supporting resources. Use skill-review when the request is only to assess a skill or propose improvements."
metadata:
...
You may use subagents during iteration to validate whether a skill works on
realistic tasks or whether a suspected problem is real. This is most useful when
you want an independent pass on the skill's behavior, outputs, or failure modes
after a revision. Only do this when it is possible to start new subagents.

When using subagents for validation, treat that as an evaluation surface. The
...
Give the minimum task-local context needed to perform the validation. Avoid
passing the intended answer, suspected bug, intended fix, or your prior
...
- "Where should I create this skill? If you do not have a preference, I will
  place it in `$CODEX_HOME/skills` (or `~/.codex/skills` when `CODEX_HOME` is
  unset) so Codex can discover it automatically."
...
```bash
scripts/init_skill.py create <skill-name> --path <output-directory> [--resources scripts,references,assets] [--examples]
```

Examples:

```bash
scripts/init_skill.py create my-skill --path "${CODEX_HOME:-$HOME/.codex}/skills" --yes
scripts/init_skill.py create my-skill --path "${CODEX_HOME:-$HOME/.codex}/skills" --resources scripts,references --yes
scripts/init_skill.py create my-skill --path ~/work/skills --resources scripts --examples --yes
```
...
```bash
scripts/generate_openai_yaml.py generate <path/to/skill-folder> --interface key=value --yes
```
...
After substantial revisions, or if the skill is particularly tricky, you should
use subagents to forward-test the skill on realistic tasks or artifacts. When
doing so, pass the artifact under validation rather than your diagnosis of what
is wrong, and keep the prompt generic enough that success depends on
transferable reasoning rather than hidden ground truth.
...
```bash
scripts/quick_validate.py validate <path/to/skill-folder>
```
...
After testing the skill, you may detect the skill is complex enough that it
requires forward-testing; or users may request improvements.

User testing often this happens right after using the skill, with fresh context
of how the skill performed.
...
context. Subagents should _not_ know that they are being asked to test the
skill. They should be treated as an agent asked to perform a task by the user.
...
In these cases, show the user your proposed prompt and request (1) a yes/no
decision, and (2) any suggested modifictions.
...
- avoid leaving artifacts the agent can find on disk between iterations; clean
  up subagents' artifacts to avoid additional contamination.

If forward-testing only succeeds when subagents see leaked context, tighten the
skill or the forward-testing setup before trusting the result.
