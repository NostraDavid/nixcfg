...
List columns hold a variable-length list per row. Use `explode()` to turn them
...
Strings inside `then()` and `otherwise()` are **column names**, not values. Wrap
literal values in `pl.lit()`.
