...
this in its **Notes** — carrying execution into the map itself — but absent
...
illegible; names read at a glance. The id and URL don't vanish — a name wraps
its link — but they ride _inside_ the name, never stand in for it.
...
run `$setup-matt-pocock-skills` if not. Consult the tracker doc's "Wayfinding
operations" section for how _this_ repo expresses them. If no tracker has been
...
human's side of it (a grilling agent that answers its own questions has broken
this).
...
1. **Name the destination.** Run a `$grilling` and `$domain-modeling` session to
   pin down what this map is finding its way to — the spec, decision, or change.
...
5. **Fire the research subagents.** For each `research` ticket you just created,
   spin up a `$research` subagent to resolve it in parallel, capturing its
   findings on a throwaway `research/<name>` branch with a context pointer from
   the ticket.
...
5. Add newly-surfaced tickets (create-then-wire); graduate any fog the answer
   ...
   specified** so it lives only as its new ticket. If the answer reveals a
