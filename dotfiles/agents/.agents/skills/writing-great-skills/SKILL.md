...
- A **model-invoked** skill lets the agent fire it autonomously _and_ lets other
  skills reach it (you can still type its name too). It contributes to **context
  load** through its trigger description. Mechanics: keep
  `policy.allow_implicit_invocation` true or omit it in `agents/openai.yaml`,
  and write a model-facing description with rich trigger phrasing ("Use when the
  user wants…, mentions…").
- A **user-invoked** skill is only reached when you type its name. It spends
  **cognitive load** because _you_ are the index that must remember it exists.
  Mechanics: set `policy.allow_implicit_invocation: false` in
  `agents/openai.yaml`; keep the required `description` as a short human-facing
