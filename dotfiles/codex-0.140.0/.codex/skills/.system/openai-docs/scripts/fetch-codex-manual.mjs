...
const parseCurlHeaders = (rawHeaders) => {
  ...
  if (!statusMatch) {
    throw new ManualFetchError(`Could not parse HTTP status from curl response: ${statusLine}`);
  }
...
const requestManualWithFetch = async (url, { method, timeoutMs }) => {
  if (typeof fetch !== "function") {
    throw new ManualFetchError("Native fetch is unavailable in this Node runtime.");
  }
...
const requestManual = async (url, { cacheDir, method, timeoutMs }) => {
  ...
  for (const transport of transports) {
    try {
      ...
      if (!response.ok) {
        throw new ManualFetchError(`${method} ${url} failed with HTTP ${response.status}.`);
      }
      return response;
    } catch (error) {
      lastError = error;
...
const fetchCodexManual = async ({
  ...
  if (getHeaderSha256 !== expectedSha256) {
    throw new ManualFetchError(`${HASH_HEADER} changed between HEAD and GET for ${manualUrl}.`);
  }
